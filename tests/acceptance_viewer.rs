...
fn criterion_7_w_toggles_wrap_without_moving_the_top_of_the_window() {
    ...
    //
    // One key at a time, each one confirmed on screen before the next is
    // sent. The status line carries the top line and the cursor's offset, so
    // it changes on every `Down` the viewer has actually *answered*; without
    // waiting for that, this loop reads a screen the viewer has not painted
    // yet, decides it is not there and presses again. On a loaded machine
    // that walks the window straight past the line it is looking for and
    // parks it at the end of the file, where no amount of waiting will bring
    // the line back.
    for _ in 0..200 {
        if s.status_row().contains("line 4/") {
            break;
        }
        let before = s.status_row();
        s.send(keys::DOWN);
        s.wait_now("the viewer to answer the key", |s| s.status_row() != before);
    }
