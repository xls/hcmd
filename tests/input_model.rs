...
#[test]
fn a_miss_keeps_the_buffer_and_flashes_the_status() {
    let mut app = app_with(&["alpha", "thorin"]);
    type_text(&mut app, "tho");
    let before = cursor(&app);
    type_text(&mut app, "z");
    assert_eq!(buffer(&app), "thoz", "the buffer is kept on a miss");
...
fn digit_keys_tabs_switches_tabs_while_the_buffer_is_empty() {
    ...
    let mut app = app_with_config(config, &["alpha", "2026-budget.xlsx"]);
    app.left.open_tab(VfsPath::local("/etc"), 9);
...
#[test]
fn the_mode_is_remembered_for_the_next_file_opened() {
    // "Mode is remembered per session". The session is the
    // application's, so the key that switches this file's mode is what the
    // next file opens in - which is why the memory lives on `App` and not on
    // the viewer that is about to be closed.
    use holoscommander::config::ViewerMode;
    let mut app = app_with(&["alpha"]);
    assert_eq!(
        app.viewers.mode, None,
        "nothing remembered until a key says so"
    );

    open_viewer(&mut app, "one\ntwo\n");
    app.service_viewer(3, 40);
    press(&mut app, KeyCode::Char('2'), NONE);
    assert_eq!(app.viewers.mode, Some(ViewerMode::Hex));
    press(&mut app, KeyCode::F(4), NONE);
    assert_eq!(
        app.viewers.mode,
        Some(ViewerMode::Text),
        "`F4` toggling counts as choosing, the same as `1` does"
    );
}
