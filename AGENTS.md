...
```sh
./scripts/gate.sh pre-push
```

That is `gate-green` plus the suite again, repeatedly, pinned to four CPUs.

### Why a green gate is not yet a green CI

`gate-green` runs the suite once on whatever this machine has. A GitHub runner
has four cores; a developer workstation here has thirty-two. A test that races
finishes in a different order under contention, and a test that measures
wall-clock time has a third as much of it. Two tests have reached master green
locally and failed on the runner for exactly that reason, and each cost a
push, a wait and a red badge to discover.

`scripts/flake-hunt.sh` exists for this and predates both of them. `pre-push`
just makes running it the default rather than something to remember. It takes
minutes rather than seconds. That is the price of not finding out from CI.

A test that only fails under contention is not a flaky test to be retried - it
is a race, in the test or in the product, and the product side is the one that
reaches users.
