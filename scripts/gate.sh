...
pre-push)
  # What to run before pushing, as opposed to before committing.
  #
  # `gate-green` runs the suite once, on whatever this machine has. That is
  # the right check for "did I break something", and it is not enough for
  # "will CI agree": a GitHub runner has four cores and this workstation has
  # thirty-two, so a test that races finishes in a different order there and a
  # test that measures wall-clock time has a third as much of it. Two tests
  # have now reached master green locally and failed on the runner for exactly
  # that reason.
  #
  # So this adds the repeated, CPU-pinned run that flake-hunt exists for. It
  # takes minutes rather than seconds, which is the price of not learning
  # about a race from a red badge after the push.
  "$0" gate-green || exit 1
  ./scripts/flake-hunt.sh "${ROUNDS:-3}" "${CPUS:-4}" || exit 1
  echo "PRE-PUSH-OK gate green, and the suite survived ${ROUNDS:-3} rounds on ${CPUS:-4} CPUs"
  ;;
...
release-assets)
  # The version comes from Cargo.toml, which is the one place it is written.
  # Hardcoding it here meant this gate kept checking the previous release and
  # passing while the new one was empty.
  ver=$(awk -F'"' '/^version/ { print $2; exit }' Cargo.toml)
  [ -n "$ver" ] || { echo "cannot read the version from Cargo.toml"; exit 1; }
  have=$(gh release view "v$ver" --json assets --jq '.assets[].name' 2>/dev/null || true)
  [ -n "$have" ] || { echo "no release v$ver, or its assets cannot be read"; exit 1; }
  missing=""
  for want in \
    "hcmd-$ver-x86_64-unknown-linux-gnu.tar.gz" \
    "hcmd-$ver-aarch64-unknown-linux-gnu.tar.gz" \
    "hcmd-$ver-x86_64-unknown-linux-musl.tar.gz" \
    "hcmd-$ver-aarch64-unknown-linux-musl.tar.gz" \
    "hcmd-$ver-x86_64-apple-darwin.tar.gz" \
    "hcmd-$ver-aarch64-apple-darwin.tar.gz" \
    SHA256SUMS
  do
...
install-sh)
  ...
  rm -rf "$dir"
  case "$v" in
    "hcmd $ver"*) echo "INSTALL-SH-OK installed and ran: $v" ;;
...
install-npx)
  ...
  rm -rf "$dir"
  case "$v" in
    "hcmd $ver"*) echo "INSTALL-NPX-OK installed and ran: $v" ;;
    *) echo "the installed binary did not report its version: $v"; exit 1 ;;
  esac
  ;;
