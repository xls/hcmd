...
///
/// **A character that would match nothing is refused rather than typed.** The
/// buffer keeps whatever last matched, so there is nothing to undo: in a
/// directory of `alpha` and `bravo`, typing `a` then `z` leaves the search at
/// `a` and the cursor on `alpha`.
///
/// This used to append the character and flash, on the reasoning that
/// `Backspace` would return to the last query that matched. That argument
/// defeats itself - refusing the character means never having left it, and it
/// costs the user a keystroke to undo something they did not ask for. The
/// theme picker already worked this way and its comment claimed the panel did
/// too; now that is true.
///
...
pub(super) fn type_into_quick_search(app: &mut App, c: char) {
    ...
    // leaves the cursor alone when it answers false, so a refusal moves
    // nothing.
    if !app.move_cursor_to_match(&candidate) {
