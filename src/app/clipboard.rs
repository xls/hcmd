...
impl App {
    /// Queue `text` for the system clipboard, reporting `count` paths.
    ///
    /// The write itself is an `OSC 52` sequence and belongs to the event loop,
    /// which owns the terminal; this only records what to send.
    pub fn queue_clipboard(&mut self, text: String, count: usize) {
        self.pending_clipboard = Some(text);
        self.message = Some(match count {
            1 => "path copied".to_string(),
            n => format!("{n} paths copied"),
        });
    }

    /// Take what a keystroke queued for the clipboard.
