...
use image::{DynamicImage, ImageEncoder as _, ImageFormat, ImageReader};
...
pub fn encode(
    ...
    match format {
        ...
        ImageFormat::Jpeg => {
            // JPEG has no alpha channel. Flattening to RGB here is what stops
            // a transparent PNG becoming a picture with a black background
            // decided by whatever happened to be in the alpha byte.
