...
fn two_jobs_writing_one_destination_name_do_not_share_a_partial_file() {
    ...
    // Both jobs carry a standing overwrite policy, because whether they
    // overlap is a fact about the machine and not about what this is
    // testing. On a machine with cores to spare they run at the same time
    // and neither sees a destination in the way; on a loaded one the second
    // starts after the first has renamed its file into place, finds
    // `movie.mkv` there and - with no policy - asks a UI that these threads
    // do not have, which is a wait with no end to it. The policy answers the
    // question in advance and leaves the race this test is about untouched.
    let spec_a = {
        let mut spec = copy_spec(vec![a], &dest);
        spec.options.conflict = Some(ConflictChoice::Overwrite);
        spec
    };
    let spec_b = {
        let mut spec = copy_spec(vec![b], &dest);
        spec.options.conflict = Some(ConflictChoice::Overwrite);
        spec
    };
    let one = std::thread::spawn(move || drive(spec_a));
    let two = std::thread::spawn(move || drive(spec_b));
