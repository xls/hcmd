...
fn fields(status: &Status) -> Vec<Field> {
    ...
    push(
        ...
        match status.mode {
            ...
            ViewerMode::Render => match status.render {
                Some(kind) => format!("render [{}]", kind.label()),
...
mod tests {
    ...
    #[test]
    fn the_colouring_follows_the_cursor_because_the_template_is_applied_there() {
        let mut a = with_viewer("0123456789abcdef0123456789abcdef");
        let area = Rect::new(0, 0, 80, 8);
        {
            let viewer = a.viewer_mut().expect("a viewer");
        ...
        assert_eq!(after.len(), before.len(), "still four bytes wide");
        assert_ne!(before, after, "and it moved with the cursor");
    }
