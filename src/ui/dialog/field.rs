...
impl Field {
    ...
    /// Draw the field as present but not editable.
    ///
    /// The live [`Self::render`] pads to the full width in `style.input()`,
    /// and that filled run is the only thing that reads as a box on screen.
    /// Drawing a greyed field as bare text therefore made an empty one
    /// vanish - the resize dialog showed `[ ] prefix filename` with nothing
    /// after it, so there was no way to tell a field was waiting behind the
    /// checkbox. This keeps the box and dims it, which is what "off" should
    /// look like.
    pub fn render_disabled(&self, f: &mut Frame, area: Rect, style: &DialogStyle) {
        if area.width == 0 || area.height == 0 {
            return;
        }
        let width = usize::from(area.width);
        let body = text::slice_columns(self.line.text(), 0, width);
        let padded = text::fit_left(&body, width, text::Crop::End, super::ellipsis(style.ascii));
        let dim = style.input().add_modifier(Modifier::DIM);
        f.render_widget(
            Paragraph::new(Line::from(Span::styled(padded, dim))).style(dim),
            area,
        );
    }
