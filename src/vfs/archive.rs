...
impl ArchiveFs {
    ...
    pub fn with_format(
        ...
        let epoch = idx.epoch();
        spawn_index(container, format, idx, epoch);
        Self { inner }
    }
...
struct FinishOnDrop {
    index: Arc<Index>,
    /// The build this guard speaks for. A guard whose build has been thrown
    /// away by a write says nothing: the rebuild that replaced it is the one
    /// that owns the status now.
    epoch: u64,
}

impl Drop for FinishOnDrop {
    fn drop(&mut self) {
        self.index.finish_epoch(
            self.epoch,
            IndexStatus::Failed(
...
fn spawn_index(
    ...
    let spawned = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let outcome = {
                let mut sink = index::Builder::for_epoch(
                    Arc::clone(&idx),
                    format.backslash_separators(),
                    epoch,
                );
    ...
    if let Err(err) = spawned {
        ...
        reporter.finish_epoch(
            epoch,
            IndexStatus::Failed(format!("could not start the archive reader: {err}")),
        );
    }
}
...
impl ArchiveFs {
    ...
    fn edit(&self, edits: &[MemberEdit]) -> Result<()> {
        ...
        if outcome.is_ok() {
            ...
            // does not have.
            let epoch = self.inner.index.invalidate();
            spawn_index(
