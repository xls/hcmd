...
impl Capabilities {
    ...
    /// SMB2 and SMB3.
    ///
    /// `seekable` is true because SMB2 READ carries an offset, so the viewer
    /// fetches a window without reading what is before it; `random_access` is
    /// false for the reason it is false over SFTP - a round trip per window is
    /// possible rather than cheap. `can_execute` is false because a file on a
    /// share has no local path to hand the kernel, and no `Entry` from that
    /// backend is a symlink, because SMB reparse points are not read.
    pub const SMB: Self = Self {
        writable: true,
        seekable: true,
        random_access: false,
        has_directories: true,
...
pub trait Vfs: Send + Sync {
    ...
    fn parent_row(&self, path: &VfsPath) -> Option<Entry> {
        path.parent().is_some().then(Entry::parent_entry)
    }
