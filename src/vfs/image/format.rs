...
pub enum FsId {
    ...
    ExFat,
    /// NTFS. Recognised, not supported, and deliberately so: the one crate
    /// that reads it depends on `binrw` 0.11, which cargo reports as
    /// containing code a future Rust will reject. A filesystem this program
    /// cannot read is a message; a dependency that stops the program building
...
impl FsId {
    ...
    /// Whether this program can read it.
    ///
    /// True for ISO 9660, the three FATs, SquashFS and the ext family; false
    /// for everything else. It is [`FsId::backend`] answering `Some`, asked as
    /// a question rather than as a reader, and the two cannot drift because
    /// this is written in terms of that one.
    ...
    pub fn backend(self) -> Option<&'static dyn VolumeFormat> {
        match self {
            ...
            // One reader for the three, because ext2, ext3 and ext4 are one
            // format with features added: what tells them apart is the feature
            // flags in the superblock, not the layout a reader walks.
            Self::Ext2 | Self::Ext3 | Self::Ext4 => Some(&super::ext::Ext),
            Self::Squashfs => Some(&super::squashfs::Squashfs),
            Self::ExFat
            | Self::Ntfs
            | Self::HfsPlus
            | Self::Apfs
...
/// Run `work`, turning a panic inside a filesystem library into a refusal.
///
/// The three readers here are fed bytes chosen by whoever wrote the image, and
/// two of the libraries under them are not written to this crate's no-panic
/// rule: `squashfs_reader` answers `unimplemented!()` for the inode types it
/// does not model, and any parser can reach an arithmetic overflow that a
/// debug build turns into a panic. Left alone, that ends a worker thread with
/// no message; caught here, it is the same kind of refusal a damaged
/// superblock produces, and the panel says so.
///
/// This contains a library. It is not a licence to panic in this crate: the
/// clippy gate still refuses `unwrap`, `expect` and `panic!` in every file
/// here.
pub fn contained<T>(what: &str, work: impl FnOnce() -> Result<T>) -> Result<T> {
    // The guard is what keeps the panic hook from restoring the terminal and
    // printing a backtrace over a running UI for a panic that is being
    // handled here (`term::ExpectedPanics`).
    let _quiet = crate::term::ExpectedPanics::new();
    // `AssertUnwindSafe` because the sink and the writer a caller passes in are
    // `&mut`. Nothing observes them after a panic: the caller turns this error
    // into a final index status or a failed read, and what had already been
    // pushed stays pushed, which is what a partial listing is for.
    match std::panic::catch_unwind(std::panic::AssertUnwindSafe(work)) {
        Ok(outcome) => outcome,
        Err(_) => Err(Error::msg(format!(
            "{what}: the reader for this filesystem stopped on something it \
             does not understand; the image may hold an entry it cannot \
             represent"
        ))),
    }
}
