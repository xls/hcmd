...
pub struct Status {
    ...
    /// Which renderer mode 3 is showing, when it is showing one.
    pub render: Option<render::RenderKind>,
...
pub struct Viewer {
    ...
    ///
    /// Chosen by hand from the picker. The file information dialog matches a
    ...
    /// The cursor `template_spans` was built for.
    ///
    /// The freshness key rather than a dirty flag: the cursor is written from
    /// some thirty places across four modules and a flag every one of them had
    /// to remember to set would be wrong within a month. Comparing the value
    /// cannot be forgotten.
...
impl Viewer {
    ...
    /// Apply a template, or take the applied one away.
    ///
    ...
    /// Rebuild [`Viewer::template_spans`] if the answer can have changed.
    ///
    /// Called once from [`Viewer::layout`], which is the one place every path
    /// through the program passes before a frame is drawn. It returns without
    /// allocating on every frame where the cursor has not moved, so the cost
    /// of an applied template is one `u64` comparison a frame plus one short
    /// rebuild per cursor movement - never anything per byte.
    pub(crate) fn refresh_template_spans(&mut self) {
        let Some(template) = self.template.as_ref() else {
            return;
        };
        if self.template_at == Some(self.cursor) {
            return;
        }
        let len = self.source.len().unwrap_or(u64::MAX);
        self.template_spans = template::extents(template, self.cursor, len);
        self.template_at = Some(self.cursor);
    }
