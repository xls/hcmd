...
/// The shortest magic that may claim a file **nobody asked about**.
///
/// A one-byte magic matches one file in 256. DER's is the single byte `0x30`,
/// which is also the digit `0`, so every text file beginning with a zero was
/// being labelled a certificate the moment it was opened in hex. That is not a
/// bug in the template - `0x30` really is how a DER sequence starts, and the
/// picker applies it by hand quite correctly - it is a bug in letting so
/// little evidence speak unprompted.
///
/// Two bytes is one in 65,536, which is thin for a single file and sound
/// across a directory of them.
pub const AUTO_MATCH_MIN_MAGIC: usize = 2;

/// Which template claims `head` without having been asked.
///
/// [`best_match`] with the evidence bar of [`AUTO_MATCH_MIN_MAGIC`] applied.
/// Every automatic path goes through here - the colouring hex mode applies on
/// its own, the format mode 3 falls back to, and the mode a file opens in -
/// so there is one answer to "is this recognised enough to act on".
#[must_use]
pub fn auto_match<'a>(templates: &'a [Template], head: &[u8]) -> Option<&'a Template> {
    best_match(templates, head).filter(|found| {
        found
            .magic
            .as_ref()
            .is_some_and(|magic| magic.bytes.len() >= AUTO_MATCH_MIN_MAGIC)
    })
}
