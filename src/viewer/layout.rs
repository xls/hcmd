...
impl Viewer {
    ...
    /// Decode the field the cursor is standing in, from the same window.
    ///
    /// Guarded by the binary search over the spans first, so a cursor outside
    /// every field costs one lookup and no decode at all. Where it is inside
    /// one, [`template::applied`] is what decodes it - the same formatter the
    /// field list and the summary use, so the status line cannot disagree with
    /// them about what a byte says.
    fn read_field_at_cursor(&mut self, w: &source::Window) {
        self.field_reading = None;
        let Some(template) = self.template.as_ref() else {
            return;
        };
        if template::field_at(&self.template_spans, self.cursor).is_none() {
            return;
        }
        let Some(base) = self.template_at.filter(|base| *base >= w.at()) else {
            return;
        };
        let Ok(at) = usize::try_from(base.saturating_sub(w.at())) else {
            return;
        };
        let readings = template::applied(template, w.bytes(), at);
        let cursor = self.cursor.saturating_sub(w.at());
        if let Some(found) = template::field_at(&readings, cursor) {
            self.field_reading = Some(format!("{}: {}", found.name, found.value));
        }
    }
