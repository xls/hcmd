...
///
/// 1. **A file whose format is recognised opens as what it is** - a JSON file
///    as a tree, a PNG as its dimensions and colour type. `document` is that
///    question already answered, and it is false whenever
///    `viewer.open_as_document` is off, which is how the setting turns this
///    step off entirely.
/// 2. **A binary file opens in hex**, because the alternative is a screenful
///    of replacement glyphs.
/// 3. **`viewer.default_mode`**, the floor: what is left when neither of the
///    two above applies, which is every ordinary text file.
///
/// # Why the session is not consulted
///
/// It used to be, and the comment here argued for it: a user who switched to
/// hex once meant it for the next file too. The user disagreed, and on
/// reflection so does the rest of this design - **the mode is a reading of the
/// file, and the next file is a different file.** Carrying hex from a `.bin`
/// onto the `README.md` opened after it is carrying an answer to a question
/// nobody asked twice. A manual switch still holds for as long as that file is
/// open; what ends is it following the user around.
pub const fn initial_mode(default_mode: ViewerMode, document: bool, binary: bool) -> ViewerMode {
    if document {
        return ViewerMode::Render;
    }
    if binary {
...
mod tests {
    ...
    #[test]
    fn a_recognised_format_opens_as_a_document_then_a_binary_in_hex() {
        use ViewerMode::{Hex, Render, Text};
        // 1. A format the program can describe wins over everything below it,
        //    including over a binary that would otherwise be hex - a PNG is a
        //    binary and opens as its dimensions rather than as its bytes.
        assert_eq!(initial_mode(Text, true, false), Render);
        assert_eq!(initial_mode(Text, true, true), Render);
        assert_eq!(initial_mode(Hex, true, true), Render);
        // 2. Then a binary, whose alternative is a screen of replacement
        //    glyphs.
        assert_eq!(initial_mode(Text, false, true), Hex);
        // 3. Then the floor, which is what `viewer.default_mode` sets.
        assert_eq!(initial_mode(Text, false, false), Text);
        assert_eq!(initial_mode(Hex, false, false), Hex);
    }

    /// `viewer.open_as_document = false` is spelled by the caller passing
    /// `document = false`, and the two steps below it are unchanged.
    #[test]
    fn with_the_document_view_off_the_floor_and_the_binary_rule_decide() {
        use ViewerMode::{Hex, Text};
        assert_eq!(initial_mode(Text, false, false), Text, "a JSON file");
        assert_eq!(initial_mode(Text, false, true), Hex, "a PNG");
        assert_eq!(
            initial_mode(Hex, false, false),
            Hex,
