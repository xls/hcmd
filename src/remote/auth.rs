...
impl AuthPlan {
    ...
    /// (the table).
    ///
    /// A host whose protocol does not verify a host key is an FTP host, and it
    /// gets [`AuthPlan::for_ftp`]'s plan instead: FTP has no agent and no keys,
    /// and offering it either would be an attempt that cannot be made.
    pub fn for_host(host: &SavedHost, home: &Path) -> Self {
        if !host.protocol.verifies_host_key() {
            return Self::for_ftp(Some(host));
        }
    ...
    pub fn for_line(parsed: &Parsed, home: &Path) -> Self {
        if !parsed.target.protocol.verifies_host_key() {
            return Self {
                methods: ftp_methods(&parsed.target.user, false),
            };
        }
        let mut methods = vec![Method::Agent];
        methods.extend(
    ...
    /// FTP's plan, which has exactly one entry: [`Method::Password`], or
    /// [`Method::Stored`] then [`Method::Password`] for a host that opted in.
    /// An anonymous login has none.
    ///
    /// `None` is a quick-connect line whose user is not anonymous;
    /// [`AuthPlan::for_line`] answers the anonymous case itself, because a line
    /// carries the user and this argument does not.
    pub fn for_ftp(host: Option<&SavedHost>) -> Self {
        let methods = match host {
            Some(host) => ftp_methods(&host.username, host.auth == AuthMethod::Keyring),
...
/// The FTP rule in one place: `anonymous` or an
/// empty user logs in with the password `anonymous@` and is never prompted, so
/// its plan is empty; anything else is a password, preceded by the stored one
/// when the user opted in.
fn ftp_methods(user: &str, stored: bool) -> Vec<Method> {
...
mod tests {
    ...
    fn ftp_has_one_method_and_an_anonymous_login_has_none() {
        ...
        assert_eq!(AuthPlan::for_ftp(Some(&ftp)).methods(), &[Method::Password]);

        ftp.auth = AuthMethod::Keyring;
        assert_eq!(
            AuthPlan::for_ftp(Some(&ftp)).methods(),
            &[Method::Stored, Method::Password]
        );

        ftp.username = "anonymous".to_string();
        assert!(AuthPlan::for_ftp(Some(&ftp)).methods().is_empty());
