        ...
        match self {
            ...
            Self::UnknownScheme(scheme) => write!(
                ...
                "{scheme}: not a protocol this program speaks; \
                 use sftp, ftp, ftps, ftps-implicit or smb"
            ),
...
/// The user an SMB line with no user in it logs in as.
///
/// `guest` rather than an empty name because that is the account a home NAS
/// actually has: `//nas/Media` is meant to open, not to raise a password
/// prompt nobody can answer. [`crate::remote::smb::connect::is_guest`] reads
/// it back as a login that carries no credential, so it is never prompted.
const GUEST: &str = "guest";

/// `//nas.local/Media` and `\\nas.local\Media` as the line already spells
/// them, turned into the same `host/path` the rest of this module reads.
///
/// A leading `//` or `\\` is SMB and nothing else - no other protocol here
/// is written that way - so the scheme is not guessed, it is read. Backslashes
/// become slashes because that is the same path written twice; a `\` inside a
/// `DOMAIN\user` is untouched, because that form has no leading `\\` and
/// never reaches this function.
fn unc(line: &str) -> Option<String> {
    let rest = line
        .strip_prefix("//")
...
/// `ftp://anonymous@ftp.example.org`, `smb://thorin:pass@192.168.1.10/Media`,
/// `//nas.local/Media`, `thorin@buildbox`, `buildbox`.
///
///
...
///    `user` for the SSH family, `anonymous` for FTP and `guest` for SMB.
...
///    not a port.
/// 6. A line beginning `//` or `\\` is SMB written the way a mount table and
///    Explorer write it, and its first path component is the share.
pub fn parse(line: &str, default: Protocol, user: &str) -> std::result::Result<Parsed, ParseError> {
    let line = line.trim();
    if line.is_empty() {
        return Err(ParseError::Empty);
    }
    let unc_form = unc(line);
    let line = unc_form.as_deref().unwrap_or(line);
    let (scheme, rest) = split_scheme(line)?;
    // A UNC line names its protocol by its shape, and that beats the scheme
    // this line does not have and the default it was not asking for.
    let protocol = unc_form
        .as_ref()
        .map(|_| Protocol::Smb)
        .or(scheme)
        .unwrap_or(default);
    ...
    let name = match name.map(str::trim).filter(|n| !n.is_empty()) {
        ...
        None if protocol == Protocol::Smb => GUEST.to_string(),
...
pub fn redact(line: &str) -> String {
    let (head, rest) = match line.split_once("://") {
        ...
        // A UNC line's leading separator is part of the head and not of the
        // authority. Without this the authority looked like the empty string
        // before the first `/`, no `@` was found in it, and a line that did
        // carry a password was returned untouched - the one outcome this
...
mod tests {
    ...
    /// SMB is a new scheme on the line the program already has, not a new
    /// syntax: `smb://` is read exactly as `sftp://` is, and the share is the
    /// first component of the path.
    #[test]
    fn an_smb_line_is_the_same_line_with_a_new_scheme() {
        let full = sftp("smb://thorin:hunter2@192.168.1.10/Media/Photos");
        assert_eq!(full.target.protocol, Protocol::Smb);
        assert_eq!(full.target.host, "192.168.1.10");
        assert_eq!(full.target.port, 445);
        assert_eq!(full.target.user, "thorin");
        assert_eq!(full.target.dir.as_deref(), Some("/Media/Photos"));
        assert_eq!(
            full.password.as_ref().and_then(Secret::expose_str),
            Some("hunter2")
        );

        // `cifs` is the same protocol under the older name.
        assert_eq!(
            sftp("cifs://nas.local/Media").target.protocol,
            Protocol::Smb
        );

        // A port is still a port, and a domain's backslash is left alone.
        let ported = sftp("smb://CORP\\thorin@nas.local:4450/Media");
        assert_eq!(ported.target.port, 4450);
        assert_eq!(ported.target.user, "CORP\\thorin");
    }

    #[test]
    fn an_smb_line_with_no_user_is_a_guest_login() {
        let parsed = parse("smb://nas.local/Media", Protocol::Sftp, "thorin").expect("parses");
        assert_eq!(parsed.target.user, "guest");
        assert!(parsed.password.is_none());
    }

    /// A bare `//server/share` is SMB by its shape, whatever the default
    /// protocol is, and the Windows spelling of the same thing is too.
    #[test]
    fn a_unc_line_is_smb_without_a_scheme() {
        for line in ["//nas.local/Media/Photos", "\\\\nas.local\\Media\\Photos"] {
            let parsed = parse(line, Protocol::Ftp, "thorin").expect("parses");
            assert_eq!(parsed.target.protocol, Protocol::Smb, "{line}");
            assert_eq!(parsed.target.host, "nas.local", "{line}");
            assert_eq!(parsed.target.port, 445, "{line}");
            assert_eq!(parsed.target.user, "guest", "{line}");
            assert_eq!(
                parsed.target.dir.as_deref(),
                Some("/Media/Photos"),
                "{line}"
            );
        }

        // The server on its own, with no share, is a legal line: it opens on
        // the list of shares.
        let bare = parse("//nas.local", Protocol::Sftp, "thorin").expect("parses");
        assert_eq!(bare.target.protocol, Protocol::Smb);
        assert_eq!(bare.target.dir, None);
    }
