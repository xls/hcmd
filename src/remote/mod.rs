//! Remote connections: SFTP and FTP/FTPS behind the ordinary [`Vfs`] trait.
//!
//!
...
//! are used on the calling blocking thread. [`fs::RemoteFs`] is the single
//! `Vfs` implementation over either of them, so the mapping from `Vfs` to a
//! remote protocol is written once and the two protocols cannot drift apart.
//!
...
impl Protocol {
    ...
    pub const ALL: &'static [Self] = &[Self::Sftp, Self::Ftp, Self::Ftps, Self::FtpsImplicit];
    ...
    pub const fn verifies_host_key(self) -> bool {
        match self {
            Self::Sftp => true,
            Self::Ftp | Self::Ftps | Self::FtpsImplicit => false,
        }
    }
